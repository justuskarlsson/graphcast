...
import xarray
from graphcast.solar_radiation import get_toa_incident_solar_radiation
    ...
    if isinstance(dt, datetime.date) and isinstance(dt, datetime.datetime):
        ...
        if "T" in dt:
            ...
            return datetime.datetime.combine(
                isodate.parse_date(dt), datetime.datetime.min.time()
            )
...
def getSingleAndPressureValues():
    ...
    singlelevel = singlelevel.rename(
        columns={
            ...
            for ind, col in enumerate(singlelevel.columns.values.tolist())
        }
    )
    singlelevel = singlelevel.rename(
        columns={"geopotential": "geopotential_at_surface"}
    )
    ...
    pressurelevel = xarray.open_dataset(
        "pressure-level.nc", engine=scipy.__name__
    ).to_dataframe()
    pressurelevel = pressurelevel.rename(
        columns={
            col: pressurelevelfields[ind]
            for ind, col in enumerate(pressurelevel.columns.values.tolist())
        }
    )
...
def addDayProgress(secs, lon: str, data: pd.DataFrame):
    ...
    prxlon = {
        lon: prog for lon, prog in list(zip(list(lons), progress.tolist()))
    }
    data["day_progress_sin"] = data.index.get_level_values(lon).map(
        lambda x: math.sin(2 * pi * prxlon[x])
    )
    data["day_progress_cos"] = data.index.get_level_values(lon).map(
        lambda x: math.cos(2 * pi * prxlon[x])
    )
...
def getSolarRadiation(longitude, latitude, dt):
    ...
    solar_radiation = (
        get_radiation_direct(dt, altitude_degrees)
...
def integrateSolarRadiation(data: pd.DataFrame):
            ...
            list(
                map(
                    lambda coord: {
                        ...
                        "lon": coord[1],
                        "lat": coord[0],
                        "toa_incident_solar_radiation": getSolarRadiation(
                            coord[1], coord[0], dt
                        ),
                    },
                    coords,
    ...
    return pd.merge(
        data, values, left_index=True, right_index=True, how="inner"
    )
...
def getTargets(dt, data: pd.DataFrame):
    ...
    lat, lon, levels, batch = (
        sorted(data.index.get_level_values("lat").unique().tolist()),
        sorted(data.index.get_level_values("lon").unique().tolist()),
        sorted(data.index.get_level_values("level").unique().tolist()),
        data.index.get_level_values("batch").unique().tolist(),
    )
    time = [
...
def getForcings(data: pd.DataFrame):
    ...
    forcingdf = data.reset_index(level="level", drop=True).drop(
        labels=predictionFields, axis=1
    )
    forcingdf = pd.DataFrame(
        index=forcingdf.index.drop_duplicates(keep="first")
    )
...
if __name__ == "__main__":
    ...
    values["inputs"] = pd.merge(
        pressure, single, left_index=True, right_index=True, how="inner"
    )
